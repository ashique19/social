...
try {
	...
	$get_event = $db->query(sprintf("SELECT events.*, cover_photo.source as event_cover_full, users.user_name, users.user_firstname, users.user_lastname FROM events LEFT JOIN posts_photos as cover_photo ON events.event_cover_id = cover_photo.photo_id INNER JOIN users ON events.event_admin = users.user_id  WHERE events.event_id = %s", secure($_GET['event_id']) )) or _error("SQL_ERROR_THROWEN");
