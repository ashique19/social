	...
	switch ($_GET['do']) {
		...
		case 'auto-connect':
			...
			$db->query(sprintf("UPDATE system_options SET 
						...
						auto_like_pages = %s,
						auto_join = %s,
						auto_join_groups = %s ", secure($_POST['auto_friend']), secure($_POST['auto_friend_users']), secure($_POST['auto_follow']), secure($_POST['auto_follow_users']), secure($_POST['auto_like']), secure($_POST['auto_like_pages']), secure($_POST['auto_join']), secure($_POST['auto_join_groups']) )) or _error("SQL_ERROR_THROWEN");
			/* return */
