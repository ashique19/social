...
try {
	...
	switch ($_GET['type']) {
		...
		case 'points':
			...
			switch ($system['points_payment_method']) {
				case 'paypal':
					$valid_methods = array('paypal');
					break;
			...
			/* insert payment */
			$db->query(sprintf("INSERT INTO points_payments (user_id, email, amount, method, time, status) VALUES (%s, %s, %s, %s, %s, '0')", secure($user->_data['user_id'], 'int'), secure($_POST['email']), secure($_POST['amount']), secure($_POST['method']), secure($date) )) or _error("SQL_ERROR_THROWEN");
			/* update user balance*/
